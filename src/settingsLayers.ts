...
export const DANGEROUS_TOP_KEYS = [
  ...
  'availableModels',
  // headlessMaxTurns 可被拉到天上（单轮无限烧 token 与时长）、headlessThinking 可悄悄开思考（成本翻倍）；
  // 两者都是成本面，不可信仓库不得设置。
  'headlessMaxTurns', 'headlessThinking',
] as const
...
function parsePresent(raw: any): Record<string, unknown> {
  ...
  if (typeof raw.headlessMaxTurns === 'number' && raw.headlessMaxTurns > 0) p.headlessMaxTurns = raw.headlessMaxTurns
