...
方括号里依次是当前模型、权限模式（`default`/`acceptEdits`/`plan`/`auto`/`dontAsk`/`yolo`，详见[权限模式](/usage/permissions)）、开了 thinking 时追加的思考档位；方括号外是当前目录名与 git 分支（有则显示）；focus 视图打开时行尾会追加一个 `· focus` 徽标。
...
- 末尾 `¥花费` 是本会话累计花费，达到 `costWarnCNY` 阈值会变色提醒。
