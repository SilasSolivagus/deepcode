...
- **收工前的验证自检门**（flag `verificationAgent` 门控，默认关，只在 headless）。改动达到验证
  合同的触发条件、却全程没派过验证子代理时，在它即将收工的那一刻催一次，给出两条合法出路：
  派一个验证者，或在最终回答里明说「未通过验证」。**刻意做成有上限的催促而不是硬拦**——合同
  自己允许没拿到 PASS 时收工（只要明说），硬拦会与合同自相矛盾，且模型若铁了心不派，门会把
  剩余轮次全烧光。默认最多催 2 次。

## 0.14.1 — 2026-08-04

### 修复
- **子代理烧穿轮次预算被截断时，父代理分辨不出来。** 此前撞上限只会封一句中性的
  「（已达最大轮数上限，已停止。）」，与「正常收工但没多说什么」无从分辨。实测后果：一个验证
  子代理被截断、从未给出结论，父代理却照常收工并声称「已验证」——**有这套机制反而比没有更糟**，
  因为交付物挂上了一个没人挣来的「已验证」标题。现在撞上限会前置明确警示，写明这不是结论、
  不得据此声称已完成或已验证。中断（用户主动打断）不加这段——那不是预算问题。

### 变更
- 子代理的轮次预算改为可按类型配置（`AgentDefinition.maxTurns`，默认仍是 30）。
  `verification` 类型设为 50，并在其系统提示里明写「把独立检查并成一轮批量执行」——实测根因
  不是「30 太少」：跑成的那次干活最多却只用 9 轮，因为它批量发；失败的是一轮一条。
